...
- [x] 1.6 写入 Agent 引导式 CI 接入设计（权限边界选项 2，2026-05-29；后续已收敛为服务端主导，业务仓 CI job 移除）
...
- [x] 2.6 同步 `cr-before-commit.mdc`；**不**安装 common `/commit`（避免覆盖团队版）；**不**复制规则到业务仓
- [x] 2.7 更新 `aicr-local`：有问题禁止写 pass、禁止 Agent 擅自改代码；`/cr` 与 hook 解耦
...
- [x] 3.9 飞书通知（辅）接口接入 → **移出本 change 范围**（follow-up，不阻塞归档）
...
- [x] 4.5 安装后验收改文档化 `--self-check`（`install.sh` 不自动跑自检；已移除 `RUN_SMOKE`）
- [x] 4.6 troubleshooting 增加 MR 覆盖率端到端调试章节
- [x] 4.7 OpenSpec 与实现对齐：去掉 `group` 枚举、明确 `/cr` 解耦与规则不下沉、补 telemetry 场景

## 5. 验收与灰度

- [x] 5.1 验证「已执行 `/cr`（pass）后提交」路径
- [x] 5.2 验证「未执行 `/cr` 直接提交」触发阻断；显式跳过可继续
- [x] 5.3 验证 runtime `--self-check`；MR 覆盖率 **GitLab 端到端待试点**（见 5.4）
- [x] 5.4 试点仓库灰度与 MR comment 覆盖率可见性 → **移出本 change 范围**（运营/试点 follow-up）

## 6. 2026-06 升级（C 主、B 兜底）

- [x] 6.1 设计并实现平台批量改造脚本（`dry-run/apply`、幂等、失败报告；入口为 `--repos-file`）
- [x] 6.2 落地 repo-bundled runtime 目录规范（`vendor/aicr-runtime`）
- [x] 6.3 改造薄入口（`.githooks/{pre-commit,post-commit,pre-push}`）为本地 runtime 跳转，禁止在线拉取
- [x] 6.4 编写安装脚本 `install.sh`（幂等、dry-run；无 `.aicr-migration-backup/`）
- [x] 6.5 在 `AI-CodeReview` 服务实现 MR 覆盖率聚合与 GitLab 发布主链路（服务仓侧）
- [x] 6.6 试点仓执行 batch rollout → **移出本 change 范围**（与 5.4 同为扩面 follow-up）
