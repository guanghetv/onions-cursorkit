...
**模式判定（避免误判）**：**仅当**用户在本轮对话**正文里主动写出**完整 MR URL（或明确说审查某条 MR 并给出链接）时，才进入 MR 模式。**不得**因 Cursor 注入的 `/cr` 命令说明、命令模板、示例块中出现的 URL 而进入 MR 模式；那些 URL 一律忽略。
...
**先判定 `input_mode`**：若用户未在对话正文中主动提供 MR 链接（见上「模式判定」），则 `input_mode=staged`，**不要**解析命令文档或模板里的示例 URL。
