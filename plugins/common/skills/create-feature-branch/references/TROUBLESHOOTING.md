...
| 重试后仍缺少规划迭代字段 | 连续两次查询返回中无 `规划迭代` 字段 | 中断流程，提示用户排查飞书字段配置、访问权限或MCP服务状态 |
| 规划迭代字段为空 | 规划迭代数组为空或无工作项ID | 提示开发者补充/修复规划迭代并中断流程，禁止创建分支 |
| 获取迭代名称失败 | 查询迭代工作项失败 | 必须先重试1次；重试后仍失败则中断流程，提示开发者排查飞书/MCP后再试 |
