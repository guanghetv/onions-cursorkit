...
**强约束（MUST）**：
- 请求参数中必须包含 `规划迭代` 字段，禁止只查 `名称` 和 `ID`
- 如果返回中缺少 `规划迭代`，必须使用同参数重试一次
- 如果重试后仍缺少 `规划迭代`，必须立即中断流程并提示用户排查飞书数据或MCP权限/服务状态
- 只有拿到 `规划迭代` 的明确值后，才能进入步骤5
...
```
❌ 无法继续：重试后仍未获取到 `规划迭代` 字段
请先排查飞书任务字段配置、访问权限或MCP服务状态后重试
```
...
**处理逻辑：**
- 正常路径：主任务查询拿到 `规划迭代` -> 提取首个 `工作项 ID` -> 查询迭代详情 -> 使用返回的迭代名称
- 如果规划迭代字段为空数组 `[]`：提示开发者先在飞书中关联规划迭代，并立即中断创建分支流程
- 如果规划迭代字段的 `工作项 ID` 为空：提示开发者修复飞书迭代数据，并立即中断创建分支流程
- 如果查询迭代详情失败：先重试1次，仍失败则提示开发者排查飞书/MCP后重试，并立即中断创建分支流程
- 如果查询成功：使用返回的迭代名称
...
```
❌ 无法继续：查询迭代详情失败（重试后仍失败）
原因：[错误详情]
请先排查飞书权限/MCP服务状态后重试
```

**迭代门禁（MUST）**：
- `规划迭代` 字段重试后仍缺失、`规划迭代=[]`、`工作项 ID 为空`、`迭代详情查询重试后仍失败` 均必须中断
- 禁止因“无法确定字段路径”伪造迭代值继续流程
- 禁止未查询迭代详情就生成分支名
