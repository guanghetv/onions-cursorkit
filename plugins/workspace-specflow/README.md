...
| 5稿结构化增强 | `/pm-spec-5` | 内审/交互评审前；允许 `[待定]`；确认前硬门禁 `/prd-risk`；未 v9 同步时可 push 飞书 |
| 产品需求风险预检 | `/prd-risk` | 独立报告 `prototypes/prd-risk-precheck.md`；HIT/待补阻断 5稿确认 |
...
**典型顺序**：`/req-new` → `/pm-proto`（可选）→ `/pm-spec-5`（确认前自动 `/prd-risk`）→ 交互评审 → `/pm-spec`（或 `/prd-publish`）→ 通知测试与开发。
