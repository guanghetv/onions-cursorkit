...
3. **`backend-*.md`（GitLab 叙事后端 spec，如有）** — 业务场景与接口叙述对照
4. **`backend-yapi-*.md`（YApi 落盘，如有）** — **请求/响应字段与类型** 的权威对照（决策 3a）

**冲突处理**：

- 若前端实现、后端行为或前端 spec 与 **测试 spec** 不一致，在验证报告与结论中**必须显著提示**（见报告模板「与测试 spec 的差异」小节），不得以「仅前端 spec 为准」默默忽略测试 spec。
- 若 **`backend-yapi-*` 字段** 与 **测试 spec** 不一致：E2E **验收仍以测试 spec 为准**，报告中须增加「⚠️ 与 YApi 字段冲突」小节，列出 YApi 与 qa 的差异，不得静默采用 YApi 覆盖 qa 验收结论。
- 静态对照接口字段时：有 `backend-yapi-*` 则**优先以 YApi 落盘**核对 mock/实现中的字段名与类型；无 YApi 时再退化为 `backend-*` 或 proposal API 契约。
...
3. 读取 `backend-yapi-*.md` 中的字段契约（如有）；读取 `backend-*.md` 中与 UI/接口结果相关的叙述（如有）
4. 对比生成三类清单：
...
| **与 spec 一致性（静态）** | 基于 `qa-*` / 前端 spec / `backend-yapi-*`（字段）/ `backend-*` 做**文档与代码的静态对照**（能确定的接口、路由、文案等），记录疑点供浏览器验证 |
...
| 依据 | 前端 spec Scenario | **测试 spec** 优先；否则前端 spec；**backend-yapi-*.md** 字段对照；**backend-*.md** 辅助场景 |
