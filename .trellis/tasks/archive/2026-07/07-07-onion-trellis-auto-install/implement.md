...
## 2. gitignore 整目录忽略规则落地为可执行文案

- [x] 2.1 在新小节中写清 `design.md`「gitignore 更新范围」表格（按平台列出整目录忽略条目：`--cursor`→`.cursor/`、`--claude`→`.claude/`、`--codex`→`.codex/`）与追加规则（去重、只追加本次选中平台、末尾追加+注释、不动 `.agents/skills/`）。
      验证点：人工核对表格条目与 `design.md` 一致；确认采用的是整目录忽略写法（与 Decision 6 一致）。
...
- [x] 4.1 `plugins/onion-sdd/USAGE.md` §8.2 后补充"Tier 2+/3 首次触发且未装 Trellis 时会询问安装"的说明段落，及 gitignore 整目录追加行为的一句话概述。
...
- [x] 5.1 汇总核对：`full-change/SKILL.md`、`onsf-auto.md`、`USAGE.md`、飞书文档、（如适用）`README.md`、`DESIGN-SUPPLEMENT.md` 六处关于本次新行为的描述用词是否一致（安装询问的触发条件、auto 模式不触发、gitignore 整目录追加），不要求逐字相同但语义不能冲突。
